...
BendingMap::~BendingMap(){}
BendingMap::BendingMap(){}
BendingMap::BendingMap(double length,double radius,double aperture,double gamma)//mm,m,mm
{
  this->length=length;
...
void BendingMap::Map()
{
  angle=length/radius;			//deg
  h=angle/abs(radius)/abs(angle);	//mm-1
  kx=abs(h);				//mm-1
  ky=0;
  //length=abs(radius)*angle;		//mm
//xx
  R(0,0)=cos(kx*length);
...
//xz
  R(0,4)=0;
  R(0,5)=h*(1-cos(kx*length))/kx/kx/1000;
  R(1,4)=0;
